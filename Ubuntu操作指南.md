...
- 多个动作逐个串行执行；当前动作失败后不继续执行后续动作。
- 左转或右转的单轮总角度最多 `360°`；超过上限会清空动作并播报原因。
- 前进和后退支持明确提出的 `1～10` 步，由移动原子动作重复组合。
- ASR 标记为日语（例如 `language=<|ja|>`）时，当前轮会被丢弃，不调用 DeepSeek、不执行动作，然后重新监听。
...
程序会丢弃明确标记为日语的 ASR 结果，并在连续模式下自动重新开始监听。日语识别属于 R1 ASR 侧问题，不是关节角度脚本造成的。
...
`read_ready_pose.py` 用于读取站立时的 13 个关节角度。`dds_robot_modified.py` 是后续绝对角度开发版本，默认运行入口仍是 `r1_agent/dds_robot.py`，替换前请先备份并在无风险动作下验证。
