 ...
 public:
  ...
  void CheckSafety() const {
    ...
    if (std::abs(rpy[0]) > 0.35f || std::abs(rpy[1]) > 0.35f) {
      throw std::runtime_error("IMU roll/pitch exceeded shoulder-trial envelope");
    }
  }
...
int main(int argc, char const* argv[]) {
  ...
  std::cout << "Confirm: environment clear, emergency stop in hand, robot stable, "
               "and this right-shoulder-pitch " << offset_radians
            << " rad trial is authorized. Type START: ";
  ...
  ShoulderTrial trial;
  return trial.Run(offset_radians) ? 0 : 2;
}
